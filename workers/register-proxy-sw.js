// MisakaNet Register + Data Proxy — Cloudflare Worker (Service Worker format)
// 部署方式: GitHub Actions 自动部署 (用 wrangler 或 API)
...
async function handleRegistration(request, env) {
  ...
  const resp = await fetch(`${GITHUB_API}/repos/${REPO}/issues`, {
    ...
    body: JSON.stringify({ title: "join", body: issueBody, labels: ["registration"] }),
  });
...
addEventListener("fetch", event => {
  ...
  const env = { REGISTER_TOKEN, MISAKANET_KV };

  // CORS preflight
  if (request.method === "OPTIONS") {
    return event.respondWith(new Response(null, { headers: CORS_HEADERS }));
  }
  ...
  if (request.method === "GET") {
    return event.respondWith(serveLandingPage());
  }

  // POST / — 注册
  if (request.method === "POST") {
    return event.respondWith(handleRegistration(request, env));
  }
